...
namespace MT {
  ...
  // Row-major 3x3 matrix
	...
	public:
		...
		{
			if (m_el[1][2] < Scalar(1))
			{
				if (m_el[1][2] > -Scalar(1))
				{
					yaw = Scalar2(Scalar_traits<Scalar>::atan2(-m_el[0][2], m_el[2][2]));
					pitch = Scalar2(asin(-m_el[1][2]));
					roll = Scalar2(Scalar_traits<Scalar>::atan2(-m_el[1][0], m_el[1][1]));
				}
				else 
				{
					yaw = Scalar2(Scalar_traits<Scalar>::atan2(-m_el[0][1], m_el[0][0]));
					pitch = Scalar_traits<Scalar2>::pi() * Scalar2(0.5); 
					roll = Scalar2(0.0);
				}
			}
			else
