                                    ...
                                    //cstThresh(20.0)
{
    ...
    QObject::connect( this, SIGNAL(setColor(QColor)), robot->getPart(partNum), SLOT(setColor(QColor)));
...
void Controller::setConstraintSpring(QVector< QVector< QPair< qreal, QVector<qreal> > > > evaluatedConstraints)
{
    ...
    for ( i=evaluatedConstraints.begin(); i!=evaluatedConstraints.end(); ++i) { // for the list of constraints
        ...
        for ( j=(*i).begin(); j!=(*i).end(); ++j) { // each constraint is a list of constraints in Conjunctive Normal Form (CNF)
            if ( j->first > getCstLen() ) { // if one of the constraints in the CNF is satisfied, the set is satisfied
                applySpringForce = false;
                break;
            }
            qreal dx = (getCstLen()-j->first)/getCstLen();
    ...
    for (k=springDispl.begin(); k!=springDispl.end(); ++k) {
        if (fabs(*k)>max) { max = fabs(*k); }
        b.addDouble(*k);
    }
    double intensity = 1.0/3.0 * (1-max);
    emit setColor(QColor::fromRgbF(intensity, intensity, intensity));
    setFCST(&b);
}
...
bool Controller::clearTracer( QString name )
{
    KinematicModel::BodyPart* bodyPart = robot->getPart(partNum);
    if (bodyPart) {
        QVector<KinematicModel::Marker*> markers = bodyPart->getMarkers();
...
QVector3D Controller::transformToLocal( const QVector3D& v ) {
    QVector3D ans;
    KinematicModel::BodyPart* bodyPart = robot->getPart(partNum);
    if (bodyPart)
        ans = bodyPart->transformToLocal(v);
    return ans;
}

QVector3D Controller::transformToGlobal( const QVector3D& v ) {
    QVector3D ans;
    KinematicModel::BodyPart* bodyPart = robot->getPart(partNum);
    if (bodyPart)
        ans = bodyPart->transformToGlobal(v);
    return ans;
}
