                                   ...
                                   const QXmlAttributes &attributes )
{
	...
	*******************************************************************/
		...
		try {
			...
			if ( radius > 0 || rField > 0)
			{
				PrimitiveObject* cylinder = NULL;
                PrimitiveObject* field = NULL;
                
                if (radius > 0) {
                    cylinder= new KinematicModel::Cylinder( radius, height );
                    cylinder->setSpecialEulerOrientation(axis);
                }
                
                if (rField > 0) {
                    field = new KinematicModel::Cylinder( rField, height );
	...
	*******************************************************************************/
	else if ( QString::compare(qName,"marker",caseSensitivity) == 0)
	{
        ...
        marker->createNormal( model->GHOST(), norm, 0.001, 0.1, Qt::yellow);
...
{
    if ( QString::compare(qName,"bodypart",caseSensitivity) == 0) {
        
		if ( !bodyPart->verify() ) {
			errorStr = "Body part contains null pointers. Check index assignments. When assigning manually, every index 0..n must be used.";
			return 0;
		}
	...
	{
        if (bodyPart->root_node == node) {
            //node->setFreeColor(Qt::darkYellow);
            //PrimitiveObject* primitive = new Cylinder(0.1, 1);
            //primitive->setSpecialEulerOrientation( QVector3D(1,0,0), 0 );
            //bodyPart->root_node->appendPrimitive(primitive);
        }
...
{
    if ( !bodyPart ) {
        errorStr = "Ecnountered <link> outside of <bodyPart>!!";
        return 0;
    }
	...
	try
	{
        ...
        if (link) {
            if (bodyPart->root_node==NULL){
                bodyPart->root_node=link;
            }
        } else errorStr = "Failed to allocate link.";
		return link;
	}
	catch (std::exception& e)
	{ 
...
{
    if ( !bodyPart ) {
        errorStr = "Ecnountered <joint> outside of <bodyPart>!!";
        return 0;
    }
	...
	try
	{
        ...
        if (joint) {
            if (bodyPart->root_node==NULL){
                bodyPart->root_node=joint;
            }
        } else errorStr = "Failed to allocate joint.\n";
		return joint;
	}
	catch (std::exception& e)
	{ 
