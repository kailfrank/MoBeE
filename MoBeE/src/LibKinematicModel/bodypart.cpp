...
BodyPart::BodyPart( Robot* robot, BodyPart* parentPart ) : idx(0), parentPart(parentPart), root_node(NULL)
{
...
void BodyPart::setColor(QColor c){
    QVector<KinTreeNode*> nodeList;
    root_node->getSubTree( nodeList );
    for (QVector<KinTreeNode*>::iterator i=nodeList.begin(); i!=nodeList.end(); ++i) {
        (*i)->setColor(c);
    }
}
...
QVector3D BodyPart::transformToLocal( const QVector3D& v ){
    return (root_node->getT().inverted()*QVector4D(v, 1.0)).toVector3D();
}

QVector3D BodyPart::transformToGlobal( const QVector3D& v ){
    return (root_node->getT()*QVector4D(v, 1.0)).toVector3D();
}
